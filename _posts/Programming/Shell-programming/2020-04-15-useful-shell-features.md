...
쉘 프롬프트에 `2`라고만 입력해도 이전, 이전 커서가 있던 디렉토리로 이동할 수 있다.
...
zsh에서 지원하는 alias는 무수히 더 많다. 자세한 목록은 [여기](https://github.com/ohmyzsh/ohmyzsh/wiki/Cheatsheet){:target="_blank"}서 확인하도록 하자.
...
![backward search](/assets/img/shell-programming/shell-we-dance-backward-search.png)
...
마지막으로 `히스토리 확장`을 사용하는 기능이다. `확장`은 이전에 [포스트](https://shoark7.github.io/programming/shell-programming/shell-expansions){:target="_blank"}로 따로 다룬 기억이 있다. 히스토리 확장은 그때 생략했는데 이유는 다른 확장에 비해서 그 중요도가 떨어진다고 판단했기 때문이다. 하지만 히스토리 확장도 쓸 수 있다면 굉장히 유용하기 때문에 이참에 확인하고 가자. 참고로 이 기능도 범용적이다.
