...
4가지의 상태를 유지하며 최종 정답을 경신해나가는 _find\_path_ 함수의 유사코드를 작성했다. 도시 수와 도시 간 거리를 담은 행렬을 각각 N과 D라고 한다.(이는 문제 정의에서 주어진다.) 그리고 1번 줄처럼 반환할 최종값을 무한으로 설정하고 차후 점점 줄여나갈 것이다.
